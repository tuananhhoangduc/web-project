  ...
  <body>
      ...
      <div class="container">
        ...
        <div class="service-management-container">
          <div class="add-service-block">
            <h2 id="form-title">Thêm Dịch vụ Mới</h2>
            <button class="btn primary-btn" id="show-add-service-form-btn"><i class="fas fa-plus"></i> Thêm Dịch vụ</button>
            ...
            <div id="add-service-form-container" class="add-service-form-container">
              <form id="add-service-form">
                ...
                <div class="form-group">
                  ...
                  <textarea id="service-description" name="service_description" rows="3" placeholder="Mô tả dịch vụ"></textarea>
                </div>
                <button type="submit" id="submit-btn" class="btn primary-btn"><i class="fas fa-save"></i> Lưu Dịch vụ</button>
                <button type="button" class="btn secondary-btn" id="cancel-add-service-form-btn"><i class="fas fa-times"></i> Làm mới</button>
              </form>
            </div>
          </div>
          ...
          <div class="service-list">
            ...
            <table>
              <thead>
                <tr><th>Mã Dịch vụ</th><th>Tên Dịch vụ</th><th>Giá</th><th>Thời lượng</th><th>Mô tả</th><th>Hành động</th></tr>
              </thead>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.getElementById('add-service-form').addEventListener('submit', function(e) {
            ...
            const endpoint = id ? '../../../backend/update_service.php' : '../../../backend/add_service.php';
            ...
            .then(res => {
                ...
                if(res.status === 'success') {
                    this.reset(); document.getElementById('service-id').value = '';
                    document.getElementById('form-title').innerText = 'Thêm Dịch vụ Mới';
                    document.getElementById('submit-btn').innerHTML = '<i class="fas fa-save"></i> Lưu Dịch vụ';
                    loadServices();
                }
            });
        });
        ...
        // Nút làm mới form
        document.getElementById('cancel-add-service-form-btn').addEventListener('click', function() {
    ...
    function loadServices() {
        fetch('../../../backend/api/get_services_admin.php')
        .then(res => res.json())
        .then(res => {
            ...
            if(res.status === 'success' && res.data.length > 0) {
                res.data.forEach(s => {
                    let priceFormat = new Intl.NumberFormat('vi-VN').format(s.price);
                    
                    // Mã hóa an toàn 100% để gắn vào data-attribute
                    let safeName = (s.service_name || '').replace(/"/g, '&quot;');
                    let safeDesc = (s.description || '').replace(/"/g, '&quot;');

                    tbody.innerHTML += `
                    <tr>
                        <td>DV${String(s.service_id).padStart(3, '0')}</td>
                        <td>${s.service_name}</td>
                        <td>${priceFormat} VNĐ</td>
                        <td>${s.duration || '0'} phút</td>
                        <td>${s.description || ''}</td>
                        <td>
                            ...
                            <button class="btn delete-btn" onclick="deleteService(${s.service_id})"><i class="fas fa-trash"></i> Xóa</button>
                        </td>
                    </tr>`;
                });
            } else {
    ...
    function editService(btn) {
        document.getElementById('service-id').value = btn.getAttribute('data-id');
        document.getElementById('service-name').value = btn.getAttribute('data-name');
        document.getElementById('service-price').value = btn.getAttribute('data-price');
        document.getElementById('service-duration').value = btn.getAttribute('data-duration');
        document.getElementById('service-description').value = btn.getAttribute('data-desc'); 

        document.getElementById('form-title').innerText = 'Cập nhật Dịch vụ'; 
    ...
    function deleteService(id) {
        if (confirm('Bạn có chắc chắn muốn xóa dịch vụ này không?')) {
            fetch(`../../../backend/delete_service.php?id=${id}`).then(res => res.json())
            .then(res => { alert(res.message); if(res.status === 'success') loadServices(); });
        }
    }
    </script>
  </body>
