...
function addReview() {
    ...
    $stmt = $conn->prepare("SELECT user_id FROM stylists WHERE stylist_id = ?");
    $stmt->execute([$stylist_id]);
    $stylist = $stmt->fetch(PDO::FETCH_ASSOC);
    $stylist_user_id = $stylist ? intval($stylist['user_id']) : 0;

    if (!$stylist_user_id) {
        echo json_encode(['status' => 'error', 'message' => 'Không tìm thấy tài khoản người dùng của thợ cắt']);
        return;
    }
    ...
    $stmt = $conn->prepare("
        INSERT INTO reviews (appointment_id, customer_id, stylist_id, rating, comment) 
        VALUES (?, ?, ?, ?, ?)
    ");
    $stmt->execute([$appointment_id, $customer_id, $stylist_user_id, $rating, $comment]);
